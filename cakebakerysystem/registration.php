...
function checkpass()
{
    ...
    var mobilePattern = /^[6-9][0-9]{9,11}$/;

    if (!namePattern.test(firstname)) {
        alert("First name must be only characters and less than 30 characters");
        return false;
    }

    if (!namePattern.test(lastname)) {
        alert("Last name must be only characters and less than 30 characters");
        return false;
    }

    if (!mobilePattern.test(mobilenumber)) {
        alert("Mobile number must be between 10 and 12 digits and start with 6, 7, 8, or 9");
        return false;
    }
